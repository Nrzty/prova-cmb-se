...
class ProcessOccurrenceResolveService
{
    public function process(string $eventInboxId): void
    {
        DB::transaction(function () use ($eventInboxId) {
            ...
            if ($event->type !== EventInboxType::RESOLVED->value)
            {
                throw new LogicException('EventInboxType inválido para ProcessOccurrenceResolveService.');
            }
            ...
            if ($occurrence->status !== OccurrenceStatus::IN_PROGRESS->value)
            {
                throw new LogicException('Transição inválida: ocorrência não está em progresso.');
            }
